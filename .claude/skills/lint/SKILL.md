---
name: lint
description: Health-check the vault for citation validity, broken links, orphan pages, contradictions, stale claims, and index drift. Reports; fixes only mechanical problems. Use when the user says "lint", "health check", "check the vault", or before a brief.
---
...
**1. Citation validity** *(the automated guard, and the one that matters)*
...
individually, since this metric is how the project is judged, so do not summarise it away.
...
**6. Staleness, source-relative rather than calendar-relative**
...
`summary:`, or whose `summary:` narrates the user in the third person, because the nudge email is
...
on its own, because one source at a time sees no counts, so gaps accumulate silently while the
source count grows.
...
Suggest new questions and missing sources at the end, since a lint pass is also where the vault
